...
import { fluxoDepositoB5 } from "../../../../utils/FluxoSAC";

// Estrutura do fluxo de atendimento

const FluxoB5Deposito = () => {
  const [currentNode, setCurrentNode] = useState(fluxoDepositoB5);
  ...
  const handleClear = () => {
    setCurrentNode(fluxoDepositoB5);
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center w-full gap-8 pt-[50px] px-20 mx-auto">
          ...
          {visitedNodes.map((node, index) => (
            <div
              key={index}
              className={`p-5 border flex flex-col gap-2 rounded-md border-black bg-gray-300 mb-5 w-[200px] text-center`}
            >
          ...
          {/* Pergunta atual */}
          <div
            className={`relative p-5 border flex text-white flex-col gap-4 rounded-md cursor-pointer border-black bg-primaryLight mb-5 w-[200px] text-center`}
          >
          ...
          {/* Botão para limpar tudo e voltar ao início */}
        </div>
      </div>
...
export default FluxoB5Deposito;
