...
import { fluxoSaqueJet } from "../../../../utils/FluxoSAC";
...
const FluxoJetSaque = () => {
  const [currentNode, setCurrentNode] = useState(fluxoSaqueJet);
  ...
  const handleClear = () => {
    setCurrentNode(fluxoSaqueJet);
    setVisitedNodes([]); // Limpar nós visitados
  };

  return (
    <div style={{ userSelect: "none" }} className="flex flex-col justify-between h-full pb-[50px]">
      <div>
        ...
        <div className="flex flex-wrap items-center w-full gap-8 pt-[50px] px-20 mx-auto">
          ...
          {/* Pergunta atual */}
          <div
            className={`relative p-5 border flex text-white flex-col gap-4 rounded-md cursor-pointer border-black bg-primaryLight mb-5 max-w-[250px] text-center`}
          >
